...
public class SettingsScrollList extends ContainerObjectSelectionList<SettingsScrollList.Entry> {
    ...
    public static class HeaderEntry extends Entry {
        ...
        public void renderContent(GuiGraphics graphics, int mouseX, int mouseY, boolean isHovered, float partialTick) {
            ...
            graphics.drawCenteredString(font, Component.literal(textStr), this.getContentXMiddle(), startY, HEADER_COLOR);
        }
    ...
    public static class LabelAndButtonEntry extends Entry {
        ...
        public void renderContent(@NonNull GuiGraphics graphics, int mouseX, int mouseY, boolean isHovered, float partialTick) {
            ...
            if (isHovered) {
                graphics.fill(startX, startY, startX + availWidth, startY + WIDGET_HEIGHT, 0x33FFFFFF);
            }

            int labelX = startX + LABEL_PADDING;
            int labelY = startY + (WIDGET_HEIGHT - font.lineHeight) / 2;
